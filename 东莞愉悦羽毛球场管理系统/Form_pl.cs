...
namespace Sales
{
    public partial class Form_pl : Form
    {
        ...
        private void Form_goods_Load(object sender, EventArgs e)
        {
            ...
            sqlc.CommandText = "select a.教练号,a.内容 陪练内容,a.可预约开始时间,a.可预约结束时间,a.姓名,a.性别,a.联系方式,a.备注,b.名称 场地 from 教练 a left join 场地 b on a.场地号=b.场地号";
            sql.Open();//打开数据库
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (button1.Text != "更新")
            {
                if (comboBox2.Text == "" || textBox2.Text == "" || textBox4.Text == "")
                {
                ...
                else
                {
                    ...
                    sqlc.Connection = sql;//将该查询对象的连接设置为上面的数据库连接类
                    //查询所有库存信息
                    sqlc.CommandText = "select * from 教练 where 姓名='" + textBox4.Text + "'";
                    sql.Open();//打开数据库
                    DataSet ds = new DataSet();
                    SqlDataAdapter sda = new SqlDataAdapter(sqlc);//用于填充dataset数据集的函数
                    sda.Fill(ds, "t1");//填充数据集
                    ...
                    else
                    {
                        ...
                        //插入语句
                        sqlc1.CommandText = "insert into 教练 values('" + textBox4.Text + "','" + comboBox1.Text + "','" + textBox2.Text + "','" + textBox5.Text + "','" + dateTimePicker1.Value + "','" + dateTimePicker2.Value + "','" + comboBox3.Items[comboBox2.SelectedIndex] + "','" + textBox3.Text + "')";
                        sql1.Open();//打开数据库
            ...
            else
            {
                ...
                sqlc1.Connection = sql1;//将该查询对象的连接设置为上面的数据库连接类
                //插入语句
                sqlc1.CommandText = "update 教练 set 姓名='" + textBox4.Text + "',性别='" + comboBox1.Text+"',内容='" + textBox2.Text + "',联系方式='" + textBox5.Text + "',可预约开始时间='" + dateTimePicker1.Value + "',可预约结束时间='" + dateTimePicker2.Value + "',场地号='" + comboBox3.Items[comboBox2.SelectedIndex] + "',备注='" + textBox3.Text + "' where 教练号='" + textBox4.Tag + "'";
                sql1.Open();//打开数据库
                int result = sqlc1.ExecuteNonQuery();//执行语句返回影响的行数
                if (result > 0)//如果执行成功则返回1
                {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                ...
                {
                    textBox4.Tag = dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                if (MessageBox.Show("确认要删除当前信息吗？", "提示框", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    sqlc.CommandText = "delete from 教练 where 教练号='" + textBox4.Tag + "'";
                    sql.Open();//打开数据库
